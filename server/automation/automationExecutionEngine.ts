...
import * as trafficIsolationService from '../system/trafficIsolationService';
      ...
      case 'negative_keyword': {
        ...
        if (negApiSuccess) {
          try {
            ...
            log.warn(`[AutoExec] v266: 否定词本地DB写入失败(API已成功): ${(dbErr as Error).message}`);
          }
        }
        ...
        if (harvestApiSuccess) {
          ...
          if (!validKeywordId || validKeywordId === '0') {
            ...
            try {
              ...
              log.warn(`[AutoExec] v357: 搜索词收割本地DB写入失败(API已成功): ${(dbErr as Error).message}`);
            }
          }
        }

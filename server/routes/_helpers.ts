...
export function calculateTrendSummary(data: any[]) {
  ...
  const totalImpressions = data.reduce((sum: number, d: any) => sum + d.impressions, 0);
  const totalClicks = data.reduce((sum: number, d: any) => sum + d.clicks, 0);
  const totalSpend = data.reduce((sum: number, d: any) => sum + d.spend, 0);
  const totalSales = data.reduce((sum: number, d: any) => sum + d.sales, 0);
  const totalOrders = data.reduce((sum: number, d: any) => sum + d.orders, 0);
  ...
  const calcTrend = (metric: string) => {
    const firstAvg = firstHalf.reduce((sum: number, d: any) => sum + (d[metric] || 0), 0) / (firstHalf.length || 1);
    const secondAvg = secondHalf.reduce((sum: number, d: any) => sum + (d[metric] || 0), 0) / (secondHalf.length || 1);

...
export async function calculateDynamicTimeout(
  ...
  try {
    ...
    const rows = (recentSyncs as Record<string, any>[])?.[0] || recentSyncs;
    ...
    // 计算平均耗时
    const avgDuration = rows.reduce((sum: number, r: Record<string, any>) => sum + (r.duration_ms || 0), 0) / rows.length;

...
export async function getSLOMetrics(): Promise<SLOMetrics> {
  ...
  try {
    ...
    try {
      ...
      `);
      const stats = ((taskStats as Record<string, any>[])?.[0] || taskStats)?.[0];
      if (stats) {
    ...
    try {
      ...
      if (Array.isArray(coverageRows) && coverageRows.length > 0) {
        ...
        metrics.dataCoverage.healthyAccounts = coverageRows.filter(
          (r: Record<string, any>) => Number(r.data_days) >= 10
        ).length;
    ...
    try {
      const freshnessResult = await database.execute(sql`
        ...
        WHERE created_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)
      `);
      const freshness = ((freshnessResult as Record<string, any>[])?.[0] || freshnessResult)?.[0];
    ...
    try {
      ...
      `);
      const latencyRows = (latencyResult as Record<string, any>[])?.[0] || latencyResult;
      if (Array.isArray(latencyRows) && latencyRows.length > 0) {
        const durations = latencyRows.map((r: Record<string, any>) => Number(r.duration_ms) / 60000); // 转为分钟
    ...
    try {
      const shardStats = await database.execute(sql`
        ...
        GROUP BY status
      `);
      const shardRows = (shardStats as Record<string, any>[])?.[0] || shardStats;
      if (Array.isArray(shardRows)) {
        for (const row of (shardRows as any[])) {
    ...
    try {
      const lockStats = await database.execute(sql`
        ...
        FROM sync_locks
      `);
      const lockRow = ((lockStats as Record<string, any>[])?.[0] || lockStats)?.[0];
      if (lockRow) {
...
export async function getSLOTrend(days: number = 7): Promise<Array<{
  ...
  try {
    ...
    const rows = (trendData as Record<string, any>[])?.[0] || trendData;

...
export interface TargetingDecision {
  /** 决策动作 */
  action: 'CREATE_KEYWORD' | 'CREATE_PRODUCT_TARGET' | 'CREATE_NEGATIVE_KEYWORD' | 'MONITOR' | 'SKIP';
  /** 投放的词或ASIN */
  targetValue: string;
  /** 关键词匹配方式 */
...
export function decideTargeting(data: SearchTermPerformance): TargetingDecision {
  const { searchTerm, clicks, impressions, orders, spend, sales, 
          campaignTargetingType, targetAcos } = data;
  ...
  // ========== Step 6: ASIN投放决策 ==========
  if (isAsin) {
    return decideAsinTargeting(data, cvr, acos, orders, clicks, dataMaturity, valueLevel);
  }
  
  // ========== Step 7: 自动广告活动特殊处理 ==========
  // 自动广告活动不能添加正面关键词，只能否定
...
export function batchDecideTargeting(
  ...
  for (const st of searchTerms) {
    ...
    switch (decision.action) {
      ...
      case 'CREATE_NEGATIVE_KEYWORD':
        result.negatives.push(decision);
  ...
  console.log(`[TargetingAlgorithm] 批量决策完成: 总计${result.summary.total}个搜索词`);
  ...
  console.log(`  否定关键词: ${result.summary.negatives}, 观察中: ${result.summary.monitors}, 跳过: ${result.summary.skipped}`);

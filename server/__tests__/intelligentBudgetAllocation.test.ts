...
describe('智能预算分配服务', () => {
  describe('多维度评分系统', () => {
    ...
    it('应该正确计算稳定性得分', () => {
      ...
      const mean = roasValues.reduce((a: unknown, b: unknown) => a + b, 0) / roasValues.length;
      const variance = roasValues.reduce((sum: number, val: Record<string, unknown>) => sum + Math.pow(val - mean, 2), 0) / roasValues.length;

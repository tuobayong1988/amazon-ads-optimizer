...
export async function enqueueBatch(tasks) {
  let enqueued = 0;
  let skipped = 0;
...
export async function dequeueMySQLFallback() {
  try {
    const conn = await getDirectConnection(1e4);
    try {
...
export async function completeTask(taskId, accountId, tier2) {
  const redis = getRedis();
  if (redis && isRedisAvailable()) {
    try {
      const pipeline = redis.pipeline();
...
export async function failTask(taskId, error48, task) {
  const redis = getRedis();
  if (redis && isRedisAvailable()) {
    try {
...
export async function saveTaskCheckpoint(checkpoint) {
  const redis = getRedis();
...
export async function loadTaskCheckpoint(taskId) {
  const redis = getRedis();
...
export async function recoverInterruptedTasks(maxAgeMs = 3 * 60 * 1e3) {  // v619: reduced from 5min to 3min for faster recovery
...
export async function cleanupStuckTasks(maxAgeMs = 20 * 60 * 1e3) {  // v619: reduced from 30min to 20min
  const redis = getRedis();
...
export async function updateTaskHeartbeat(taskId, update) {
  const redis = getRedis();
...
export async function persistProcessingTasks() {
  const redis = getRedis();
  if (!redis || !isRedisAvailable()) return 0;
  try {
    const all3 = await redis.hgetall(QUEUE_KEYS.processing);
...
export async function createShards(taskId, totalRecords, shardSize = 5e3) {
  const shards = [];
...
export async function updateShardStatus(taskId, shardId, status, processedCount) {
  const redis = getRedis();
  if (redis && isRedisAvailable()) {
    try {
...
export async function getCompletedShards(taskId) {
...
export async function acquireAccountLock(accountId) {
  const redis = getRedis();
...
export async function releaseAccountLock(accountId) {
  const redis = getRedis();
  if (!redis || !isRedisAvailable()) return;
  try {
    const key = ACCOUNT_LOCK_PREFIX + accountId;
...
export async function renewAccountLock(accountId) {
  const redis = getRedis();
  if (!redis || !isRedisAvailable()) return false;
  try {
    const key = ACCOUNT_LOCK_PREFIX + accountId;
...
export function stopWorkerLifecycle() {
  if (heartbeatTimer2) {
...
export async function clearStaleAccountLocks() {
  const redis = getRedis();
  if (!redis || !isRedisAvailable()) return 0;
  try {

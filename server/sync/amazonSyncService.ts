...
export class AmazonSyncService {
  ...
  async syncAll(options?: { performanceDays?: number; syncMode?: 'init' | 'daily' | 'recovery'; layers?: number[]; retryFailedLayers?: boolean }): Promise<{
    ...
    // v660: 步骤级独立超时配置 — 大幅放宽，确保大账户也能完成
    // v659实测发现: 否定词3分钟、建议竞价5/10分钟、绩效报告10/20分钟对大账户均不够
    // 核心原则: 稳定性和成功率优先于效率，宁可等待也不要超时失败
    const STEP_TIMEOUT_CONFIG: Record<string, number> = {
      // v663: 全面提升步骤超时，确保大账户每个步骤都有足够时间完成
      // 广告活动/广告组/关键词等列表API：通常几秒完成，给30分钟（从10分钟提升）
      'default': 30 * 60 * 1000,
      // 否定词同步：v662实测90107的SP否定关键词/商品定位20分钟不够，给45分钟
      'negative': 45 * 60 * 1000,
      // 搜索词/定向报告：需要异步报告，给45分钟（从30分钟提升）
      'report': 45 * 60 * 1000,
      // 绩效数据同步：最耗时，需要多天报告，给60分钟（从45分钟提升）
      'performance': 60 * 60 * 1000,
      // 建议竞价：批量API调用，给45分钟（从25分钟提升）
      'bidRecommendation': 45 * 60 * 1000,
      // 广告位绩效：异步报告，给45分钟（从25分钟提升）
      'placement': 45 * 60 * 1000,
    };

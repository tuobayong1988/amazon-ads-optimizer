...
export class AmazonSyncService {
  ...
  async syncAll(options?: { performanceDays?: number; syncMode?: 'init' | 'daily' | 'recovery'; layers?: number[]; retryFailedLayers?: boolean }): Promise<{
    ...
    const STEP_TIMEOUT_CONFIG: Record<string, number> = {
      // 广告活动/广告组/关键词等列表API：通常几秒完成，给10分钟
      'default': 10 * 60 * 1000,
      // 否定词同步：v659实测3分钟不够，大账户否定词数量多，给20分钟
      'negative': 20 * 60 * 1000,
      // 搜索词/定向报告：需要异步报告，给30分钟
      'report': 30 * 60 * 1000,
      // 绩效数据同步：最耗时，需要多天报告，v659实测10/20分钟不够，给45分钟
      'performance': 45 * 60 * 1000,
      // 建议竞价：批量API调用，v659实测5/10分钟不够，给25分钟
      'bidRecommendation': 25 * 60 * 1000,
      // 广告位绩效：异步报告，给25分钟
      'placement': 25 * 60 * 1000,
    };

...
export async function acquireReportSemaphore(accountId, campaignCount = 0, stepName = "unknown", heartbeatCallback, isManualSync = false) {
...
export function releaseReportSemaphore(accountId) {
...
export function getReportSemaphoreStatus() {
  const now = Date.now();
...
export function resetReportSemaphore() {

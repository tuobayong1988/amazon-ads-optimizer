...
import { calculateUCB, calculateMinExplorationBudget, needsReEvaluation } from "../algorithm/algorithmUtils";
import { 
  campaigns, 
  performanceGroups, 
  dailyPerformance,
  ...
  for (const campaign of (campaignList as unknown[])) {
    ...
    try {
      ...
      const dailyDataForWeighting: timeDecayService.DailyRawData[] = rawDailyData.map(d => ({
        ...
        date: typeof d.date === 'string' ? d.date : new Date(d.date as Record<string, unknown>).toISOString(),

...
export function detectOpportunityWindow(
  ...
  const avgHistCpc = sameHourSignals.reduce((s: any, h: any) => s + h.avgCpc, 0) / sameHourSignals.length;
  const avgHistImpressions = sameHourSignals.reduce((s: any, h: any) => s + h.avgImpressions, 0) / sameHourSignals.length;
  const avgHistCompetition = sameHourSignals.reduce((s: any, h: any) => s + h.avgCompetition, 0) / sameHourSignals.length;

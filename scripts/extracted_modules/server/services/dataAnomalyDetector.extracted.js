...
// Original module: server/services/dataAnomalyDetector.ts
// Lines: 1787

var dataAnomalyDetector_exports = {};
__export(dataAnomalyDetector_exports, {
...
var log66;
var init_dataAnomalyDetector = __esm({
  "server/services/dataAnomalyDetector.ts"() {
    "use strict";
    init_db2();
    init_logger();
    log66 = createModuleLogger("DataAnomalyDetector");
    __name(runAnomalyDetection, "runAnomalyDetection");
  }
});
...
__name(v620RawQuery, "v620RawQuery");

// ============================================================
// P1-1: Operation Review Gateway - Human-in-the-loop review mechanism
// ============================================================
var operationReviewGateway_exports = {};
__export(operationReviewGateway_exports, {
  OperationReviewGateway: () => OperationReviewGateway,
  reviewGatewayRouter: () => reviewGatewayRouter
});
...
__name(OperationReviewGateway, "OperationReviewGateway");

var reviewGatewayRouter;
var init_reviewGateway = __esm({
  "server/services/operationReviewGateway.ts"() {
    "use strict";
    init_trpc();
    init_db2();
    init_schema2();
    init_drizzle_orm();
    reviewGatewayRouter = router({
      getPendingReviews: protectedProcedure.input(external_exports.object({
        accountId: external_exports.number()
      })).query(async ({ input }) => {
        const _db = await getDb(); return OperationReviewGateway.getPendingReviews(_db, input.accountId);
      }),
      approveOperation: protectedProcedure.input(external_exports.object({
        reviewId: external_exports.number()
      })).mutation(async ({ input }) => {
        const { syncCampaignStatusToAmazon, syncBidAdjustmentsToAmazon } = await Promise.resolve().then(() => (init_amazonApiHelper(), amazonApiHelper_exports));
        const _db = await getDb(); return OperationReviewGateway.executeApproved(_db, input.reviewId, { syncCampaignStatusToAmazon, syncBidAdjustmentsToAmazon });
      }),
      rejectOperation: protectedProcedure.input(external_exports.object({
        reviewId: external_exports.number(),
        reason: external_exports.string().optional()
      })).mutation(async ({ input }) => {
        await v620RawQuery(
          `UPDATE operation_reviews SET status = 'rejected', rejectedReason = ?, approvedAt = NOW() WHERE id = ?`,
          [input.reason || "", input.reviewId]
        );
        return { success: true };
      }),
      getStats: protectedProcedure.input(external_exports.object({
        ...
        const _db = await getDb(); return OperationReviewGateway.getReviewStats(_db, input.accountId);
      }),
      expireStale: protectedProcedure.mutation(async () => {
        const _db = await getDb(); return OperationReviewGateway.expireStaleReviews(_db);
      })
    });
  }
});
...
// ============================================================
var impactPredictor_exports = {};
__export(impactPredictor_exports, {
  ImpactPredictor: () => ImpactPredictor,
  impactPredictorRouter: () => impactPredictorRouter
});
...
__name(ImpactPredictor, "ImpactPredictor");

var impactPredictorRouter;
var init_impactPredictor = __esm({
  "server/services/impactPredictor.ts"() {
    "use strict";
    init_trpc();
    init_db2();
    impactPredictorRouter = router({
      predictBidChange: protectedProcedure.input(external_exports.object({
        keywordId: external_exports.number(),
        currentBid: external_exports.number(),
        proposedBid: external_exports.number()
      })).query(async ({ input }) => {
        const _db = await getDb(); return ImpactPredictor.predictBidChange(_db, input.keywordId, input.currentBid, input.proposedBid);
      }),
      predictCampaignPause: protectedProcedure.input(external_exports.object({
        campaignId: external_exports.number()
      })).query(async ({ input }) => {
        const _db = await getDb(); return ImpactPredictor.predictCampaignPause(_db, input.campaignId);
      })
    });
  }
});
...
// ============================================================
var coreKeywordManager_exports = {};
__export(coreKeywordManager_exports, {
  CoreKeywordManager: () => CoreKeywordManager,
  coreKeywordRouter: () => coreKeywordRouter
});
...
__name(CoreKeywordManager, "CoreKeywordManager");

var coreKeywordRouter;
var init_coreKeywordManager = __esm({
  "server/services/coreKeywordManager.ts"() {
    "use strict";
    init_trpc();
    init_db2();
    coreKeywordRouter = router({
      getCoreKeywords: protectedProcedure.input(external_exports.object({
        accountId: external_exports.number(),
        lookbackDays: external_exports.number().optional(),
        minOrders: external_exports.number().optional()
      })).query(async ({ input }) => {
        const _db = await getDb(); return CoreKeywordManager.identifyCoreKeywords(_db, input.accountId, input);
      }),
      syncCoreKeywords: protectedProcedure.input(external_exports.object({
        accountId: external_exports.number()
      })).mutation(async ({ input }) => {
        const _db = await getDb(); return CoreKeywordManager.syncCoreKeywordsToDb(_db, input.accountId);
      }),
      checkBidProtection: protectedProcedure.input(external_exports.object({
        keywordId: external_exports.number(),
        proposedBid: external_exports.number()
      })).query(async ({ input }) => {
        const _db = await getDb(); return CoreKeywordManager.checkBidAgainstProtection(_db, input.keywordId, input.proposedBid);
      })
    });
  }
});
...
// ============================================================
var healthSignalMonitor_exports = {};
__export(healthSignalMonitor_exports, {
  HealthSignalMonitor: () => HealthSignalMonitor,
  healthSignalRouter: () => healthSignalRouter
});
...
// ============================================================
var _healthSignalMemCache = new Map();
var _healthSignalPending = new Map();
var HEALTH_SIGNAL_CACHE_TTL = 5 * 60; // 5 minutes in seconds
var HEALTH_SIGNAL_REDIS_KEY_PREFIX = "hs:report:";
var HEALTH_SIGNAL_PRECOMPUTE_INTERVAL = 30 * 60 * 1000; // 30 minutes (safety net, event-driven is primary)
var _hsPrecomputeTimer = null;
...
__name(_hsRedisGet, "_hsRedisGet");
...
__name(_hsRedisSet, "_hsRedisSet");
...
__name(_hsPrecomputeAll, "_hsPrecomputeAll");
// === v620-fix10: Event-Driven Single-Account Precompute ===
var _hsEventPrecomputeInFlight = new Set();
...
__name(_hsPrecomputeForAccount, "_hsPrecomputeForAccount");

// === v620-fix10: Redis Pub/Sub Subscriber for Cross-Instance Cache Warm ===
var _hsSubInitialized = false;
...
__name(_hsInitSubscriber, "_hsInitSubscriber");
...
__name(startHealthSignalPrecompute, "startHealthSignalPrecompute");
...
__name(HealthSignalMonitor, "HealthSignalMonitor");

var healthSignalRouter;
var init_healthSignalMonitor = __esm({
  "server/services/healthSignalMonitor.ts"() {
    "use strict";
    init_trpc();
    init_db2();
    healthSignalRouter = router({
      getFullReport: protectedProcedure.input(external_exports.object({
        accountId: external_exports.number()
      })).query(async ({ input }) => {
        const _db = await getDb(); return HealthSignalMonitor.getFullHealthReport(_db, input.accountId);
      }),
      getCacheStatus: protectedProcedure.input(external_exports.object({
        accountId: external_exports.number()
      })).query(async ({ input }) => {
        const redisData = await _hsRedisGet(input.accountId);
        const memData = _hsMemGet(input.accountId);
        return {
          redis: redisData ? { available: true, generatedAt: redisData.generatedAt, computeTimeMs: redisData.computeTimeMs } : { available: false },
          memory: memData ? { available: true, generatedAt: memData.generatedAt, computeTimeMs: memData.computeTimeMs } : { available: false },
          precomputeInterval: HEALTH_SIGNAL_PRECOMPUTE_INTERVAL / 1000 + "s",
          cacheTTL: HEALTH_SIGNAL_CACHE_TTL + "s"
        };
      }),
      triggerPrecompute: protectedProcedure.mutation(async () => {
        _hsPrecomputeAll();
        return { triggered: true, message: "Precomputation started in background" };
      }),
      triggerPrecomputeForAccount: protectedProcedure.input(external_exports.object({
        accountId: external_exports.number()
      })).mutation(async ({ input }) => {
        _hsPrecomputeForAccount(input.accountId).catch(() => {});
        return { triggered: true, accountId: input.accountId, message: "Single-account precomputation started" };
      }),
      getTACoS: protectedProcedure.input(external_exports.object({
        accountId: external_exports.number(),
        days: external_exports.number().optional()
      })).query(async ({ input }) => {
        const _db = await getDb(); return HealthSignalMonitor.calculateTACoS(_db, input.accountId, input.days || 30);
      }),
      getImpressionShare: protectedProcedure.input(external_exports.object({
        accountId: external_exports.number()
      })).query(async ({ input }) => {
        const _db = await getDb(); return HealthSignalMonitor.calculateImpressionShare(_db, input.accountId);
      }),
      getBidHealth: protectedProcedure.input(external_exports.object({
        accountId: external_exports.number()
      })).query(async ({ input }) => {
        const _db = await getDb(); return HealthSignalMonitor.calculateBidHealth(_db, input.accountId);
      }),
      getCampaignHealth: protectedProcedure.input(external_exports.object({
        accountId: external_exports.number()
      })).query(async ({ input }) => {
        const _db = await getDb(); return HealthSignalMonitor.calculateCampaignHealth(_db, input.accountId);
      })
    });
  }
});
...
// ============================================================
var syncOrchestrator_exports = {};
__export(syncOrchestrator_exports, {
  SyncOrchestrator: () => SyncOrchestrator,
  syncOrchestratorRouter: () => syncOrchestratorRouter
});
...
__name(SyncOrchestrator, "SyncOrchestrator");

var syncOrchestratorRouter;
var init_syncOrchestrator = __esm({
  "server/services/syncOrchestrator.ts"() {
    "use strict";
    init_trpc();
    init_db2();
    syncOrchestratorRouter = router({
      preCheckAccount: protectedProcedure.input(external_exports.object({
        accountId: external_exports.number()
      })).query(async ({ input }) => {
        const _db = await getDb(); return SyncOrchestrator.preCheckAccount(_db, input.accountId);
      }),
      getIncrementalCandidates: protectedProcedure.input(external_exports.object({
        accountId: external_exports.number(),
        sinceHours: external_exports.number().optional()
      })).query(async ({ input }) => {
        const _db = await getDb(); return SyncOrchestrator.getIncrementalSyncCandidates(_db, input.accountId, input.sinceHours || 24);
      }),
      ...
      triggerSync: protectedProcedure.input(external_exports.object({
        accountId: external_exports.number(),
        type: external_exports.enum(["full_sync", "incremental", "performance_only"])
      })).mutation(async ({ input }) => {
        const _dbSync = await getDb();
        return SyncOrchestrator.enqueueSyncTask({
...
// ============================================================
var discoveryGovernor_exports = {};
__export(discoveryGovernor_exports, {
  DiscoveryGovernor: () => DiscoveryGovernor,
...
__name(DiscoveryGovernor, "DiscoveryGovernor");
...
__name(EventCalendar, "EventCalendar");

var discoveryRouter, eventCalendarRouter;
var init_discoveryGovernor = __esm({
  "server/services/discoveryGovernor.ts"() {
    "use strict";
    init_trpc();
    init_db2();
    discoveryRouter = router({
      classifyTarget: protectedProcedure.input(external_exports.object({
        targetId: external_exports.number(),
        targetType: external_exports.enum(["keyword", "product_target"]).optional()
      })).query(async ({ input }) => {
        const _db = await getDb(); return DiscoveryGovernor.classifyTarget(_db, input.targetId, input.targetType || "keyword");
      }),
      getStepDownSchedule: protectedProcedure.input(external_exports.object({
        currentBid: external_exports.number(),
        targetBid: external_exports.number(),
        steps: external_exports.number().optional()
      })).query(async ({ input }) => {
        return DiscoveryGovernor.getStepDownSchedule(input.currentBid, input.targetBid, input.steps || 3);
      }),
      shouldAllowDecrease: protectedProcedure.input(external_exports.object({
        targetId: external_exports.number(),
        proposedDecrease: external_exports.number()
      })).query(async ({ input }) => {
        const _db = await getDb(); return DiscoveryGovernor.shouldAllowBidDecrease(_db, input.targetId, input.proposedDecrease);
      })
    });
    eventCalendarRouter = router({
      getCurrentEvents: protectedProcedure.query(async () => {
        return EventCalendar.getCurrentEvents();
      }),
      getUpcomingEvents: protectedProcedure.input(external_exports.object({
        days: external_exports.number().optional()
      })).query(async ({ input }) => {
        return EventCalendar.getUpcomingEvents(input.days || 30);
      }),
      shouldBlockDecrease: protectedProcedure.query(async () => {
        return EventCalendar.shouldBlockDecrease();
      }),
      addCustomEvent: protectedProcedure.input(external_exports.object({
        name: external_exports.string(),
        startDate: external_exports.string(),
        endDate: external_exports.string(),
        bidMultiplier: external_exports.number().optional(),
        budgetMultiplier: external_exports.number().optional(),
        marketplace: external_exports.string().optional()
      })).mutation(async ({ input }) => {
        const _db = await getDb(); return EventCalendar.addCustomEvent(_db, input);
      })
    });
  }
});

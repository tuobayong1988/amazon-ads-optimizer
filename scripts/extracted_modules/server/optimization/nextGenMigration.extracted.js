...
// Original module: server/optimization/nextGenMigration.ts
// Lines: 276

async function ensureNextGenTables() {
  try {
    ...
    for (const table of NEXTGEN_TABLES) {
      try {
        const [rows] = await db.execute(sql.raw(`SHOW TABLES LIKE '${table.name}'`));
        const exists2 = Array.isArray(rows) ? rows.length > 0 : false;
        if (exists2) {
          tablesExisted++;
          log184.info(`[NextGen Migration] v614i-fix23: \u8868 '${table.name}' \u5DF2\u5B58\u5728\uFF0C\u8DF3\u8FC7\u521B\u5EFA`);
        } else {
          await db.execute(sql.raw(table.ddl));
          tablesCreated++;
          log184.info(`[NextGen Migration] v614i-fix23: \u8868 '${table.name}' \u521B\u5EFA\u6210\u529F`);
        }
      } catch (err) {
        try {
          await db.execute(sql.raw(table.ddl));
          tablesCreated++;
          log184.info(`[NextGen Migration] v614i-fix23: \u8868 '${table.name}' \u901A\u8FC7\u56DE\u9000\u65B9\u5F0F\u521B\u5EFA\u6210\u529F`);
        } catch (createErr) {
          log184.warn(`[NextGen Migration] v614i-fix23: \u8868 '${table.name}' \u521B\u5EFA\u5931\u8D25:`, createErr.message);
        }
      }
    }
    log184.info(`[NextGen Migration] v614i-fix23: \u8FC1\u79FB\u5B8C\u6210 - ${tablesCreated} \u4E2A\u8868\u65B0\u5EFA, ${tablesExisted} \u4E2A\u8868\u5DF2\u5B58\u5728, \u5171 ${NEXTGEN_TABLES.length} \u4E2A\u8868`);
    return { success: true, tablesCreated };
  } catch (err) {
    return { success: false, tablesCreated: 0, error: err.message };
  }
}
var log184, NEXTGEN_TABLES;
var init_nextGenMigration = __esm({
  "server/optimization/nextGenMigration.ts"() {
    "use strict";
    init_logger();
    init_db2();
    init_drizzle_orm();
    log184 = createModuleLogger("NextGenMigration");
    NEXTGEN_TABLES = [
      {
    ...
    __name(ensureNextGenTables, "ensureNextGenTables");
  }
});

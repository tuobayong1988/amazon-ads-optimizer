...
// Original module: server/sync/tokenHealthChecker.ts
// Lines: 200

var tokenHealthChecker_exports = {};
__export(tokenHealthChecker_exports, {
  getAllTokenHealthStatus: () => getAllTokenHealthStatus,
  markTokenUnhealthy: () => markTokenUnhealthy,
  precheckToken: () => precheckToken,
  resetAllTokenHealth: () => resetAllTokenHealth,
  resetTokenHealth: () => resetTokenHealth,
  shouldSkipSync: () => shouldSkipSync
});
async function precheckToken(credentials, accountId, accountName2, marketplace) {
...
function shouldSkipSync(accountId) {
...
function getAllTokenHealthStatus() {
...
function resetTokenHealth(accountId) {
...
function resetAllTokenHealth() {
  const count11 = tokenHealthCache.size;
  tokenHealthCache.clear();
  log80.info(`[TokenHealth] \u5DF2\u91CD\u7F6E\u6240\u6709${count11}\u4E2A\u8D26\u6237\u7684Token\u5065\u5EB7\u72B6\u6001`);
  return count11;
}
function markTokenUnhealthy(accountId, accountName2, marketplace, errorType, errorMessage) {
  updateHealthStatus(accountId, accountName2, marketplace, errorType, errorMessage);
  log80.warn(`[TokenHealth] \u8D26\u6237${accountId}(${accountName2}) Token\u88AB\u6807\u8BB0\u4E3A${errorType}: ${errorMessage}`);
}
var log80, tokenHealthCache, CONFIG;
var init_tokenHealthChecker = __esm({
  "server/sync/tokenHealthChecker.ts"() {
    "use strict";
    init_logger();
    log80 = createModuleLogger("TokenHealthChecker");
    tokenHealthCache = /* @__PURE__ */ new Map();
    CONFIG = {
      // Token检查结果缓存时间（健康Token）
      HEALTHY_CACHE_TTL_MS: 30 * 60 * 1e3,
      // 30分钟
      // Token检查结果缓存时间（失效Token）
      UNHEALTHY_CACHE_TTL_MS: 60 * 60 * 1e3,
      // 1小时
      // 自动恢复尝试间隔
      AUTO_RECOVERY_INTERVAL_MS: 4 * 60 * 60 * 1e3,
      // 4小时
      // 连续失败多少次后暂停同步
      MAX_CONSECUTIVE_FAILURES: 3,
      // Token预检查超时
      PRECHECK_TIMEOUT_MS: 1e4
      // 10秒
    };
    __name(precheckToken, "precheckToken");
    __name(updateHealthStatus, "updateHealthStatus");
    __name(sendTokenAlert, "sendTokenAlert");
    __name(shouldSkipSync, "shouldSkipSync");
    __name(getAllTokenHealthStatus, "getAllTokenHealthStatus");
    __name(resetTokenHealth, "resetTokenHealth");
    __name(resetAllTokenHealth, "resetAllTokenHealth");
    __name(markTokenUnhealthy, "markTokenUnhealthy");
  }
});

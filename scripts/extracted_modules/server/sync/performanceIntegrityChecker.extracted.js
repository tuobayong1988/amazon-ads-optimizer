...
// Original module: server/sync/performanceIntegrityChecker.ts
// Lines: 284

var performanceIntegrityChecker_exports = {};
__export(performanceIntegrityChecker_exports, {
  checkAccountPerformanceCoverage: () => checkAccountPerformanceCoverage,
  checkAllAccountsCoverage: () => checkAllAccountsCoverage,
  postSyncCoverageCheck: () => postSyncCoverageCheck
});
...
async function checkAccountPerformanceCoverage(accountId) {
...
async function checkAllAccountsCoverage() {
  const conn = await getDirectConnection();
  try {
...
async function postSyncCoverageCheck(accountId, syncType) {
  try {
...
var log133;
var init_performanceIntegrityChecker = __esm({
  "server/sync/performanceIntegrityChecker.ts"() {
    "use strict";
    init_logger();
    init_connection();
    log133 = createModuleLogger("PerfIntegrity");
    __name(getFirstRow, "getFirstRow");
    __name(getAllRows, "getAllRows");
    __name(checkAccountPerformanceCoverage, "checkAccountPerformanceCoverage");
    __name(checkAllAccountsCoverage, "checkAllAccountsCoverage");
    __name(postSyncCoverageCheck, "postSyncCoverageCheck");
  }
});

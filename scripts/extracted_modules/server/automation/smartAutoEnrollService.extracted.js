...
// Original module: server/automation/smartAutoEnrollService.ts
// Lines: 288

var smartAutoEnrollService_exports = {};
__export(smartAutoEnrollService_exports, {
  autoEnrollAccount: () => autoEnrollAccount,
  postSyncAutoEnrollCheck: () => postSyncAutoEnrollCheck,
  scanAndAutoEnrollAll: () => scanAndAutoEnrollAll
});
async function autoEnrollAccount(accountId, dryRun = false) {
...
async function scanAndAutoEnrollAll(dryRun = false) {
  const db_instance = await getDb();
  if (!db_instance) {
    return {
...
async function postSyncAutoEnrollCheck(accountId, syncType) {
  try {
...
var init_smartAutoEnrollService = __esm({
  "server/automation/smartAutoEnrollService.ts"() {
    ...
    init_drizzle_orm();
    init_logger();
    log131 = createModuleLogger("SmartAutoEnroll");
    AUTO_ENROLL_CONFIG = {
      // 最低日均花费阈值（USD），低于此值的Campaign不自动纳管
      minDailySpendUSD: 1,
      // 默认优化目标
      defaultOptimizationGoal: "target_acos",
      // 默认目标ACOS（30%）
      defaultTargetAcos: "30",
      // 每个自动创建的PG最多纳管多少个Campaign
      maxCampaignsPerAutoGroup: 50,
      // 分析花费数据的天数
      spendAnalysisDays: 14,
      // 自动纳管的PG名称前缀
      autoGroupNamePrefix: "[Auto] ",
      // fix24-patch: 是否按Campaign类型分组创建PG
      groupByType: true,
      // fix24-patch: Campaign类型到PG名称的映射
      typeNameMap: {
        "sp_auto": "SP Auto",
        "sp_manual": "SP Manual",
        "sb": "SB (Brands)",
    ...
    __name(autoEnrollAccount, "autoEnrollAccount");
    __name(createGroupedPGs, "createGroupedPGs");
    __name(createSinglePG, "createSinglePG");
    __name(groupCampaignsByType, "groupCampaignsByType");
    __name(findUnmanagedHighSpendCampaigns, "findUnmanagedHighSpendCampaigns");
    __name(scanAndAutoEnrollAll, "scanAndAutoEnrollAll");
    __name(postSyncAutoEnrollCheck, "postSyncAutoEnrollCheck");
  }
});

...
// Original module: server/utils/dynamicLogLevel.ts
// Lines: 280

var dynamicLogLevel_exports = {};
__export(dynamicLogLevel_exports, {
  clearAccountDebug: () => clearAccountDebug,
  clearGlobalLevel: () => clearGlobalLevel,
  clearModuleDebug: () => clearModuleDebug,
  getDynamicLogStatus: () => getDynamicLogStatus,
  setAccountDebug: () => setAccountDebug,
  setGlobalLevel: () => setGlobalLevel,
  setModuleDebug: () => setModuleDebug,
  shouldEnableDebug: () => shouldEnableDebug,
  startDynamicLogLevelRefresh: () => startDynamicLogLevelRefresh,
  stopDynamicLogLevelRefresh: () => stopDynamicLogLevelRefresh
});
function shouldEnableDebug(moduleName, accountId) {
  const now = Date.now();
...
async function setModuleDebug(moduleName, ttlSeconds = 600) {
...
async function clearModuleDebug(moduleName) {
  try {
    const { getRedis: getRedis2, isRedisAvailable: isRedisAvailable2 } = await Promise.resolve().then(() => (init_redisClient(), redisClient_exports));
    if (isRedisAvailable2()) {
      const redis = getRedis2();
...
async function setAccountDebug(accountId, ttlSeconds = 1800) {
...
async function clearAccountDebug(accountId) {
  try {
    const { getRedis: getRedis2, isRedisAvailable: isRedisAvailable2 } = await Promise.resolve().then(() => (init_redisClient(), redisClient_exports));
    if (isRedisAvailable2()) {
      const redis = getRedis2();
...
async function setGlobalLevel(level, ttlSeconds = 300) {
...
async function clearGlobalLevel() {
  try {
    const { getRedis: getRedis2, isRedisAvailable: isRedisAvailable2 } = await Promise.resolve().then(() => (init_redisClient(), redisClient_exports));
    if (isRedisAvailable2()) {
      const redis = getRedis2();
...
async function getDynamicLogStatus() {
  const result = {
    global: null,
    modules: [],
    accounts: [],
...
function startDynamicLogLevelRefresh() {
...
function stopDynamicLogLevelRefresh() {
  if (_refreshTimer) {
...
var init_dynamicLogLevel = __esm({
  "server/utils/dynamicLogLevel.ts"() {
    "use strict";
    init_logger();
    log3 = createModuleLogger("DynamicLogLevel");
    KEY_PREFIX = "log:level:";
    KEY_GLOBAL = `${KEY_PREFIX}global`;
    KEY_MODULE_PREFIX = `${KEY_PREFIX}module:`;
    KEY_ACCOUNT_PREFIX = `${KEY_PREFIX}account:`;
    memoryOverrides = {
      global: null,
    ...
    CACHE_REFRESH_INTERVAL_MS = 1e4;
    _refreshTimer = null;
    __name(shouldEnableDebug, "shouldEnableDebug");
    __name(setModuleDebug, "setModuleDebug");
    __name(clearModuleDebug, "clearModuleDebug");
    __name(setAccountDebug, "setAccountDebug");
    __name(clearAccountDebug, "clearAccountDebug");
    __name(setGlobalLevel, "setGlobalLevel");
    __name(clearGlobalLevel, "clearGlobalLevel");
    __name(getDynamicLogStatus, "getDynamicLogStatus");
    __name(refreshCache, "refreshCache");
    __name(startDynamicLogLevelRefresh, "startDynamicLogLevelRefresh");
    __name(stopDynamicLogLevelRefresh, "stopDynamicLogLevelRefresh");
  }
});

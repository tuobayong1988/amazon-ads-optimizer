...
## Bug修复：数据同步API返回HTML错误
- [x] 检查服务器日志查找错误原因（代理超时导致）
- [x] 修复API端点返回HTML而不是JSON的问题（改为异步任务模式）
- [x] 前端改用轮询机制获取同步进度
- [ ] 验证修复后同步功能正常
...
### 测试同步功能
- [x] 在Amazon API设置页面测试同步功能
- [x] 验证异步模式是否正常工作（同步请求立即返回，不再超时）
- [x] 检查轮询机制是否正确获取进度（前端正确显示同步进度和站点状态）

### 实现时区转换
- [x] 调用GET /v2/profiles获取店铺时区
- [x] 在amazonApiCredentials表存储timezone和currencyCode
- [x] 同步开始时自动获取并更新时区信息
- [ ] AMS数据处理时将UTC转换为店铺当地时间（待实现）
- [ ] Reporting API数据保持店铺当地时间（已实现）
...
- [x] 推送当前代码到GitHub仓库
- [x] 查询Amazon广告API文档确定最长时间范围（SP: 95天, SB: 60天, SD: 65天）
- [x] 优化移动端时间线模块显示效果
...
### 已完成
- [x] 代码已推送到GitHub (2026-01-13)

## 2026-01-13 AMS实时数据流状态显示问题
- [x] 检查双轨制同步面板AMS状态显示逻辑
- [x] 确认SQS消费者服务正在运行
- [x] 修复AMS数据流状态在界面上的显示

## 2026-01-13 传统报表API状态显示问题
- [x] 检查传统报表API最后同步时间显示逻辑
- [x] 修复记录数显示（当前停留在788）
- [x] 确保显示正确的同步时间和记录数
...
## 2026-01-13 根据官方文档优化双轨制数据同步
- [x] 修复SP报表使用7d归因字段（attributedSales7d, attributedConversions7d）
- [x] 确认SB/SD报表使用14d归因字段
- [x] 优化AMS Traffic数据累加逻辑（Delta模式）
- [x] 优化AMS Budget数据快照逻辑（Snapshot模式）
- [x] 添加流式GZIP解压处理大文件
- [x] 修复cost单位转换（AMS cost已是美元单位）（微美分/1000000）

## 2026-01-13 全量数据同步问题诊断与修复
- [ ] 诊断API全量同步失败的根本原因
- [ ] 检查SP/SB/SD广告活动同步逻辑
- [ ] 检查AMS订阅配置是否包含所有数据集
- [ ] 修复API全量同步功能
- [ ] 修复AMS接收SP/SB/SD所有广告类型数据
- [ ] 验证数据同步完整性

## 组件集成与SaaS优化（用户请求）

### 组件集成
- [ ] 集成TACoS、实时标识、Sparkline到数据概览页
- [ ] 集成快速操作按钮到广告活动列表
- [ ] 集成AI归因分析到智能优化中心
- [ ] 集成浪费资金Top10到数据分析页
- [ ] 集成预算模拟器到边际效益分析页
...
### 报告任务数据库表
- [x] 创建report_jobs表存储报告任务
- [x] 添加任务状态字段（pending/submitted/processing/completed/failed/expired）
- [x] 添加重试计数和最大重试次数字段
- [x] 添加报告ID和下载URL字段

### 异步报告服务
- [x] 实现AsyncReportService类
- [x] 实现createReportJob方法创建单个报告任务
- [x] 实现createAttributionJobs方法创建归因回溯任务（SP 14天，SB/SD 30天）
- [x] 实现createInitializationJobs方法创建新店铺初始化任务（90天热数据+365天冷数据）
- [x] 实现submitPendingJobs方法提交待处理任务
- [x] 实现checkSubmittedJobs方法检查已提交任务状态
- [x] 实现processCompletedJobs方法下载和处理完成的报告
- [x] 实现getJobStats方法获取任务统计
- [x] 实现cleanupExpiredJobs方法清理过期任务

### 报告任务调度器
- [x] 实现ReportJobScheduler类
- [x] 实现定时提交任务（每30秒）
- [x] 实现定时检查状态（每60秒）
- [x] 实现定时处理完成报告（每30秒）
- [x] 实现定时清理过期任务（每24小时）
...
### API路由
- [x] 添加reportJobs路由到appRouter
- [x] 实现getStats接口获取任务统计
- [x] 实现getSchedulerStatus接口获取调度器状态
- [x] 实现runOnce接口手动触发处理周期
- [x] 实现createAttributionJobs接口创建归因任务
- [x] 实现createInitializationJobs接口创建初始化任务
- [x] 实现cleanupExpiredJobs接口清理过期任务

### 单元测试
- [x] 编写AsyncReportService测试用例
- [x] 编写ReportJobScheduler测试用例
- [x] 验证归因天数配置正确性
- [x] 验证日期切片生成逻辑
...
### 店铺初始化状态管理
- [ ] 在amazon_ad_accounts表添加初始化状态字段（initialization_status, initialization_started_at, initialization_completed_at）
- [ ] 创建初始化进度追踪表（account_initialization_progress）
- [ ] 实现初始化状态机（pending -> initializing -> completed）

### 初始化任务调度
- [ ] 新店铺接入后自动触发初始化流程
- [ ] 热数据优先（90天，7天切片，高优先级）
- [ ] 冷数据其次（91-365天，30天切片，低优先级）
- [ ] 广告结构数据同步（广告活动、广告组、关键词、定位）

### 日常同步策略优化
- [ ] 区分初始化模式和增量模式
- [ ] 初始化完成后切换到增量同步（只同步T-1天数据）
- [ ] 归因回溯只针对冻结区外数据进行校验
- [ ] AMS实时流作为主数据源，API报告作为校验

### 初始化进度UI
- [ ] 在店铺管理页面显示初始化进度
- [ ] 显示各阶段完成状态（热数据/冷数据/结构数据）
- [ ] 预估剩余时间显示
- [ ] 初始化完成通知
...
### 分层配置
- [x] 实时层（1-7天）：1天切片，全部报告类型
- [x] 热数据层（8-30天）：7天切片，全部报告类型
- [x] 温数据层（31-90天）：15天切片，Campaign+AdGroup
- [x] 冷数据层（91-365天）：30天切片，仅Campaign
...
### TypeScript错误修复
- [ ] 修复剩余8个TypeScript错误（modelPredictions表schema字段）
...
### 广告活动自动优化仪表板
- [ ] 创建自动优化仪表板页面
- [ ] 显示自动执行的优化动作（预算调整、出价调整等）
- [ ] 显示优化效果对比
- [ ] 添加优化历史记录


## Amazon API批量授权功能（第二十四阶段）

### 分析现有授权流程
- [x] 分析现有Amazon API授权流程
- [x] 分析数据结构和站点配置
...
## 批量授权页面修复与广告算法优化（第二十五阶段）

### 批量授权页面修复
- [x] 检查并修复区域选择组件不显示选项的问题
- [x] 添加紫鸟浏览器专用授权入口（中国大陆卖家）
- [x] 优化授权流程的用户体验

### 广告优化算法分析与改进
- [x] 分析Amazon API可获取的所有广告数据
- [x] 评估当前广告优化算法的实现
- [x] 识别算法改进点
- [x] 生成详细的广告优化算法改进报告
...
## Dashboard数据显示修复（2026-01-16）

### 问题诊断
- [x] 确认本地开发服务器数据显示正常
- [x] 确认数据库中有真实的daily_performance数据
- [x] 确认生产环境显示$0是因为代码版本不同步
...
## 双轨制数据同步模块全面修复 (2026-01-16 v74+)

### 传统报表API和AMS实时数据流
- [ ] 修复SQL查询中的data_source字段名问题（生产环境可能没有此字段）
- [ ] 确保查询使用正确的字段名（dataSource vs data_source）
- [ ] 修复记录数显示为0的问题
...
### MySQL语法修复
- [x] 修复dualTrackSyncService.ts中的PostgreSQL风格双引号字段名
- [x] 将 `"completedAt"` 改为 `completedAt`
- [x] 将 `"recordsSynced"` 改为 `recordsSynced`
- [x] 将 `"recordCount"` 改为 `recordCount`
- [x] 将 `"errorMessage"` 改为 `errorMessage`
- [x] 将 `"syncType"` 改为 `syncType`
- [x] 将 `"lastUpdate"` 改为 `lastUpdate`
- [x] 确保所有SQL查询与MySQL语法兼容
- [x] 部署v75到生产环境 (v75-mysql-syntax-fix-20260116051552)
- [x] 验证双轨制同步模块正常工作
- [x] 验证数据概览模块正常显示
- [x] 验证广告活动模块正常显示5532个广告活动
...
### 管理员账户权限修复
- [x] 修复管理员无法访问所有账户的问题
- [x] 修改adAccount.list路由，让管理员能够访问所有账户
- [x] 修改listWithPerformance路由，让管理员能够访问所有账户
- [x] 修改getStats路由，让管理员能够访问所有账户
- [x] 修改getDailyTrend路由，让管理员能够访问所有账户
- [x] 修改getDataDateRange路由，让管理员能够访问所有账户
- [x] 本地测试验证修复
- [ ] 部署到生产环境并验证

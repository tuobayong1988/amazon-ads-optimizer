-- NextGen Algorithm Tables Migration (v198)
-- 创建下一代算法体系所需的全部数据库表

-- 1. 上下文特征表 — 存储关键词/商品定向的多维上下文特征快照
...
-- 2. RL训练日志表 — 记录State-Action-Reward三元组
...
-- 5. 算法策略选择日志表 — 记录元学习器的策略选择决策
CREATE TABLE IF NOT EXISTS `algorithm_selection_logs` (
  `id` int NOT NULL AUTO_INCREMENT,
  `accountId` int NOT NULL,
  `keywordId` int DEFAULT NULL,
  `targetId` int DEFAULT NULL,
...
-- 6. 预算组合优化结果表 — 存储预算分配优化的决策和结果
CREATE TABLE IF NOT EXISTS `budget_optimization_results` (
  `id` int NOT NULL AUTO_INCREMENT,
  `accountId` int NOT NULL,
...
-- 7. 关键词语义图谱表 — 存储关键词/搜索词之间的语义关系
CREATE TABLE IF NOT EXISTS `keyword_semantic_graph` (
  `id` int NOT NULL AUTO_INCREMENT,
  `accountId` int NOT NULL,

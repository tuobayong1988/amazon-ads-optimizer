...
SET @preparedStatement = (SELECT IF(
  ...
  'SELECT 1',
  'CREATE INDEX idx_organization_id ON users(organization_id)'
));
PREPARE alterIfNotExists FROM @preparedStatement;
EXECUTE alterIfNotExists;
DEALLOCATE PREPARE alterIfNotExists;
...
SET @preparedStatement = (SELECT IF(
  ...
  'SELECT 1',
  'CREATE INDEX idx_organization_id ON campaigns(organization_id)'
));
PREPARE alterIfNotExists FROM @preparedStatement;
EXECUTE alterIfNotExists;
DEALLOCATE PREPARE alterIfNotExists;
...
SET @preparedStatement = (SELECT IF(
  (
    SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
    WHERE table_schema = @dbname
      AND table_name = 'performance_groups'
      AND index_name = 'idx_organization_id'
  ) > 0,
  'SELECT 1',
  'CREATE INDEX idx_organization_id ON performance_groups(organization_id)'
));
PREPARE alterIfNotExists FROM @preparedStatement;
EXECUTE alterIfNotExists;
DEALLOCATE PREPARE alterIfNotExists;

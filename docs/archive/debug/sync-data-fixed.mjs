...
async function syncData() {
  ...
  // 插入SP广告活动 - 使用正确的列名 campaignStatus
  console.log('\n🔄 插入SP广告活动到数据库...');
  let spInserted = 0;
  for (const campaign of spCampaigns) {
    try {
      const campaignType = campaign.targetingType?.toLowerCase() === 'auto' ? 'sp_auto' : 'sp_manual';
      const status = campaign.state?.toLowerCase() || 'enabled';
      const dailyBudget = campaign.budget?.budget || 0;
      
      await connection.execute(
        `INSERT INTO campaigns (accountId, campaignId, campaignName, campaignType, targetingType, dailyBudget, campaignStatus, createdAt, updatedAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        [
          creds.accountId,
          ...
          campaign.name,
          campaignType,
          campaign.targetingType?.toLowerCase() || 'manual',
          dailyBudget,
          status
        ]
      );
      ...
      console.log(`   SP插入错误: ${error.message}`);
    }
  }
  ...
  for (const campaign of sbCampaigns) {
    try {
      ...
      console.log(`   SB插入错误: ${error.message}`);
    }
  }
  ...
  for (const campaign of sdCampaigns) {
    try {
      ...
      console.log(`   SD插入错误: ${error.message}`);
    }
  }
  ...
  // 验证数据
  const [count] = await connection.execute('SELECT COUNT(*) as total FROM campaigns');
  console.log(`\n数据库中现有广告活动: ${count[0].total} 个`);
